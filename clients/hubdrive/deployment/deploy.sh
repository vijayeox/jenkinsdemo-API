...
oswald()
{
    ...
    then
        ...
        echo -e "${YELLOW}Copying EOX Apps to /opt/oxzion/eoxapps directory${RESET}"
        mkdir -p /opt/oxzion/eoxapps
        rsync -rl --delete ./Oswald /opt/oxzion/eoxapps
        echo -e "${YELLOW}Building Oswald apps using deployapp API${RESET}"
        jwt=$(curl --location --request POST 'http://localhost:8080/auth' --form 'username=admintest' --form 'password=Welcome2eox!' 2>/dev/null | jq -r '.data.jwt')
        curl --location --request POST 'http://localhost:8080/app/deployapp' -H 'Authorization: Bearer '${jwt}'' -F 'path=/opt/oxzion/eoxapps/Oswald'
        echo -e "${YELLOW}Copying EOX Apps directory Complete!${RESET}"
        echo -e "${GREEN}Building and Running package discover in bos${RESET}"
        cd /opt/oxzion/view/apps/Oswald/
        rm -rf /opt/oxzion/view/apps/Oswald/node_modules
        npm install --unsafe-perm
        npm run build
        cd /opt/oxzion/view/themes/OswaldTheme/
        rm -rf /opt/oxzion/view/themes/OswaldTheme/node_modules
        npm install --unsafe-perm
        npm run build
        chown www-data:www-data -R /opt/oxzion/view
        chown www-data:www-data -R /opt/oxzion/eoxapps
...
hubdrive()
{
    ...
    then
        ...
        echo -e "${YELLOW}Copying EOX Apps to /opt/oxzion/eoxapps directory${RESET}"
        mkdir -p /opt/oxzion/eoxapps
        rsync -rl --delete ./hubdrive /opt/oxzion/eoxapps
        echo -e "${YELLOW}Building hubdrive apps using deployapp API${RESET}"
        jwt=$(curl --location --request POST 'http://localhost:8080/auth' --form 'username=admintest' --form 'password=Welcome2eox!' 2>/dev/null | jq -r '.data.jwt')
        curl --location --request POST 'http://localhost:8080/app/deployapp' -H 'Authorization: Bearer '${jwt}'' -F 'path=/opt/oxzion/eoxapps/hubdrive'
        echo -e "${YELLOW}Copying EOX Apps directory Complete!${RESET}"
        echo -e "${GREEN}Building and Running package discover in bos${RESET}"
        cd /opt/oxzion/view/apps/HUB/
        rm -rf /opt/oxzion/view/apps/HUB/node_modules
        npm install --unsafe-perm
        npm run build
        cd /opt/oxzion/view/themes/HUBTheme/
        rm -rf /opt/oxzion/view/themes/HUBTheme/node_modules
        npm install --unsafe-perm
        npm run build
        chown www-data:www-data -R /opt/oxzion/view
        chown www-data:www-data -R /opt/oxzion/eoxapps
