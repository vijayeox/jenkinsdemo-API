...
document.addEventListener("DOMContentLoaded", function() {
    ...
    Formio.createForm(
        ...
        form.on("submit", function(submission, next) {
            ...
            submission.data.preferences = JSON.stringify({
                            ...
                            $.ajax({
                                ...
                                success: function(response) {
                                    if (response.data) {
                                        ...
                                        form
                                            .setSubmission({ data: response.data })
                                            .then((response2) => {
            ...
            if (changed && changed.changed) {
                ...
                if (properties) {
                    ...
                    if (properties["clear_field"]) {
                        var clearField = properties["clear_field"].split(",");
                        Object.keys(clearField).forEach((key) => {
                            var targetComponent = form.getComponent(clearField[key]);
                            if (targetComponent) {
