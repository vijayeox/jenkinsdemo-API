...
<body>
	<div class ="div_cover">
		...
		<p>Enclosed is the endorsement proposal to your PADI Endorsed {if $product == 'Group Professional Liability'} Professional Liability {else} Dive Center {/if}policy.{if isset($additionalNotes) && $additionalNotes != ""}  This endorsement makes the following change:</p>
		<p>{$additionalNotes}</p>
		{/if}
		...
		<p>To make this coverage change, please provide us with the following items by {if isset($quote_due_date) && $quote_due_date != ""}{$quoteDueDate|date_format:"%m/%d/%Y"}{/if}:</p>
