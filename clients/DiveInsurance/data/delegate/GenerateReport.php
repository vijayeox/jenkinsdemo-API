...
class GenerateReport extends PolicyDocument {
    ...
    { 
        ...
        $this->logger->info("The data returned from Group Professional Liability quarterly report generation is : ". print_r($files['data'], true));
        if(empty($files['data'])){
    ...
    private function newDataArray($data,$product){
        ...
        foreach ($data['data'] as $key => $value) {
            ...
            if(($product == "diveStoreProperty" || $product == "diveStore")  && isset($value['additionalLocations']) && !empty($value['additionalLocations']) && $value['additionalLocations'] != "[]"){
                ...
                if($product == "diveStore"){
                    $responsePrimary[$i]['nonDivingPool'] = $value['nonDivingPoolAmount'];
                    $responsePrimary[$i]['ExcessLiability']  = $value['ExcessLiabilityFP'];
                    $responsePrimary[$i]['TravelAgent'] = $value['TravelAgentEOFP'];
                    $responsePrimary[$i]['medicalExpense'] = $value['MedicalExpenseFP'];
                    $responsePrimary[$i]['lakeQuarry'] = $value['lakeQuarryPond'];
                    $responsePrimary[$i]['nonOwnedAuto'] = $value['Non-OwnedAutoFP'];
                }
                if($product == "diveStoreProperty"){
                    ...
                    $responsePrimary[$i]['Addl_Cnts_Limit'] = $value['dspropTotal'];
            ...
            if($product == 'groupProfessionalLiability' && isset($value['groupPL']) && !empty($value['groupPL']) && $value['groupPL'] != "[]" &&  $value['groupProfessionalLiabilitySelect'] == 'yes'){
                        ...
                        foreach ($groupData as $key1 => $value1) { 
                            if($value2['padi'] == $value1['padi']) {
                                if($value2['status'] != $value1['status']){
                                    $previous_careerCoverage[$i] = $value2;
                                    $groupPL[$i] = $value1;
                                    $i+= 1;
                                }
                            }
                ...
                foreach ($groupPL as $key2 => $value2) {
                    ...
                    $response[$i]['certificate_no'] = $group_certificate_no;//Changed from group certificate
                    $response[$i]['end_date'] = $value['end_date'];
                    $response[$i]['address1'] = $value['address1'];
                    $response[$i]['address2'] = isset($value['address2']) ? $value['address2'] : '';
                    $response[$i]['city'] = $value['city'];
                    ...
                    $response[$i]['premium'] = $i== 0? $value['groupCoverage']: "$0";
                    $response[$i]['excess_premium'] = $i== 0?$value['groupExcessLiability'] : "$0";
