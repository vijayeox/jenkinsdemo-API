...
class GetPremiumRates extends AbstractAppDelegate
{
    ...
    {
        $this->logger->info("Get Premium Coverage List -----" . print_r($data, true));
        $coverageList = array();
        $andClause = " ";
        if ($is_upgrade == 0) {
        ...
        $this->logger->info("Get Premium Coverage List Query-----" . print_r($select, true));
    ...
    private function getSubCoverageList(&$data, $product, $is_upgrade, $andClause, $persistenceService)
    {
        ...
        $this->logger->info("Get Premium Sub Coverage List Query-----" . print_r($select, true));
        $result = $persistenceService->selectQuery($select);
        while ($result->next()) {
            $rate = $result->current();
