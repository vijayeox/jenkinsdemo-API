...
use Oxzion\Service\ElasticSearch\ElasticClient;
...
class ElasticSearch{
	...
	public function __construct($avatar,$onlyaggs=0){
        $conf = new Config();
	...
	public function FilterWithParams($params){
		...
		$searchconfig['fields'] = isset($params['fields'])?$params['fields']:null;
		$searchconfig['select'] = isset($params['select'])?$params['select']:null;
		$searchconfig['start'] = isset($params['start'])?$params['start']:null;
		$searchconfig['pagesize'] = isset($params['pagesize'])?$params['pagesize']:null;
		$searchconfig['filter'] = isset($params['filter'])?$params['filter']:null;
		$searchconfig['range'] = isset($params['range'])?$params['range']:null;
		$searchconfig['group'] = isset($params['group'])?$params['group']:null;
		$searchconfig['aggregates'] = isset($params['aggregates'])?$params['aggregates']:null;
		$searchconfig['type'] = isset($params['type'])?$params['type']:null;
		$searchconfig['sort'] = isset($params['sort'])?$params['sort']:null;
		$searchconfig['listfields'] = isset($params['listfields'])?$params['listfields']:null;
		if(isset($params['searchval'])&&json_decode($params['searchval'],true)){
			$searchconfig['searchtext'] = json_decode($params['searchval'],true);
		} else {
			if(isset($params['searchval'])){
				$searchconfig['searchtext'] = $params['searchval'];
			}
		}
		if(isset($params['formid'])||isset($params['moduleid'])){
			$params['entity'] = 'form';
			$searchconfig['formid'] = isset($params['formid'])?$params['formid']:null;
			$searchconfig['moduleid'] = isset($params['moduleid'])?$params['moduleid']:null;
		}
	...
	protected function getFieldsList(&$searchconfig,$entity){
		if($entity=='instanceforms'){
			...
			if(isset($searchconfig['formid'])){
				$form = new Form($searchconfig['formid']);
				return array_column($form->getFields(),'name');
			} else {
					return array_column(Utilities::getDefaultFieldsWithTitle(),'field');
			}
		} else if($entity=='user'){
	...
	public function FilterDataByEntity($entity, &$searchconfig, $sortkey=null, $sortdir=null){
		...
		if (!$this->onlyaggs) {
			...
			if($this->avatarobj->getFlag('opendata')||$searchconfig['type']){
		...
		if(isset($searchconfig['searchtext'])){
			$boolfilterquery['query']['bool']['should']=$this->getTextQuery('',$searchconfig['searchtext'],$entity,$boolfilterquery['_source']);
		...
		if ($this->onlyaggs) {
			...
			foreach($result_obj['hits']['hits'] as $key=>$value){
				$results['results'][] = $value['_source'];
				if(isset($value['_source'][$idparam])){
					$results['results'][$key]['id'] = $value['_source'][$idparam];
				}
				if(isset($value['highlight'])){
					...
					if(count($highlightarray)>1){
						$results['highlighting'][$value['_source']['id']]['data'] = implode(" , ",$highlightarray);
					} else {
						if($entity=='formcomments'){
							$results['highlighting'][$value['_source']['instanceform_id']]['data'] = $highlightarray[0];
						} else {
							if($highlightarray){
								$results['highlighting'][$value['_source']['id']]['data'] = $highlightarray[0];
							}
						}
					}
				}
			}
			$results['start'] = $searchconfig['start'];
			$results['numFound'] = $result_obj['hits']['total'];
			$results['display_keys'] = $this->getDisplayKeys($entity);
			if(isset($searchconfig['select'])){
				$results['fields'] = $searchconfig['select'];
			}
			if(isset($searchconfig['listfields'])){
...
//		print_r($boolfilterquery);exit;
	...
	protected function getContentKeys($entity){
		switch ($entity){
			...
			return array('ID'=>'entity_id',
				'Assigned Group'=>'assignedgroup','Owner Group'=>'ownergroup',
				'Stage'=>'statusname','Assigned To'=>'assignedto',
				'Owner'=>'createdby');
			break;
			case 'formcomments':
			return array('ID'=>'instanceform_id',
				'Title'=>'title',
				"Comment"=>'comment',
				'Comment By'=>'commenting_user','Commented On'=>'date_created');
			break;
			case 'messages':
			return array('ID'=>'id','From'=>'from_user','Received On'=>'date_created','Subject'=>'subject');
			break;
			case 'ole':
			return array('ID'=>'id','Group'=>'ole_group',
				'Created By'=>'createdby_user','Group Chat'=>'ole');
			break;
			case 'user':
			return array("First Name"=>'firstname',"Last Name"=>'lastname');
			break;
			case 'attachments':
			return array('ID'=>array('instanceformid','messageid'),
				'Subject'=>'subject',
				'From'=>'from_user',
				'Assigned Group'=>'assignedgroup',
				'Owner Group'=>'ownergroup',
				'Assigned To'=>'assignedto',
				'Owner'=>'createdby','Attachment'=>'attachment.content','Filename'=>'filename');
			break;
			default:
			return array('fields'=>'*');
			break;
	...
	protected function getDefaultFilters(&$searchconfig,$entity){
		$avatar_groups = array_column($this->avatarobj->getGroups(), 'id');
		if (!$this->onlyaggs) {
		...
		if($entity=='instanceforms' || $entity=='timesheet'){
			if(isset($searchconfig['formid'])){
				...
				if(isset($searchconfig['moduleid'])){
					$appMapper = new App($searchconfig['moduleid']);
					$formlist = array_column($appMapper->getForms($searchconfig['orgid']),"id");
					$mustquery[] = array('terms'=>array('formid'=>$formlist));
				}
			}
...
//						} else 
		...
		if($this->avatarobj->getFlag('onlyme')){
			if(isset($entity_type)&&($entity_type == 'form' || $entity_type == 'formcomments')){
