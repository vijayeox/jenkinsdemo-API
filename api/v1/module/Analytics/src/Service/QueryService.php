...
class QueryService extends AbstractService
{
    ...
    public function getQueryList($params = null)
    {
        ...
        if (isset($params['show_deleted']) && $params['show_deleted'] == true) {
            ...
            $query = "SELECT q.uuid,q.name,datasource_id,q.configuration,q.ispublic,IF(q.created_by = " . AuthContext::get(AuthConstants::USER_ID) . ", 'true', 'false') as is_owner,q.version,q.account_id FROM `ox_query` as q inner join ox_datasource as d on q.datasource_id = d.id " . $where . " " . $sort . " " . $limit;
        }
