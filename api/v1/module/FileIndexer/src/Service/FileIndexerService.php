...
class FileIndexerService extends AbstractService
{
    ...
    public function getRelevantDetails($fileId)
    {
        if(isset($fileId))
        {
            ...
            GROUP BY file.id,app_name,entity.id, entity.name,file_data,file_uuid,file.latest,file.workflow_instance_id,file.is_active, file.parent_id, file.org_id,w.user_id,w.id, w.status,w.activity_instance_id,w.name, w.activities";
        ...
        try {
            ...
            if ($total > 0) {
                ...
                foreach ($batches as $batch) {
                        ...
                        GROUP BY wf_user.user_id, wf_inst.id, wf_inst.status, act_inst.activity_instance_id, wf.name) w
                        ON w.id = file.workflow_instance_id
                        where file.id in (".$fileIds.") AND app.id =".$appID." GROUP BY file.id,app_name,entity.id, entity.name,file_data,file_uuid,file.latest,file.workflow_instance_id,file.is_active, file.parent_id, file.org_id,w.user_id,w.id, w.status,w.activity_instance_id,w.name, w.activities";
                        $this->runGenericQuery("SET SESSION group_concat_max_len = 1000000;");
                        $this->logger->info("Executing Query - $select");
                        ...
                        foreach ($bodys as $key => $value) {
                            $databody = array();
                            if(!empty($value)){
                                if(isset($value['file_data'])){
                                    $file_data = json_decode($value['file_data'],true);
                                    $databody = array_merge($value,$file_data);
                                }
                                unset($databody['file_data']);
                                if(isset($databody['app_name']))
                                    $app_name = $databody['app_name'];
                            }
                            foreach ($databody as $key1 => $value) {
                                if(is_string($value))
                                {
                                    $result = json_decode($value);
                                    if (json_last_error() === JSON_ERROR_NONE) {
                    ...
                    if (isset($app_name)&&isset($bodys) && count($bodys) > 0) {
                        $this->messageProducer->sendTopic(json_encode(array('index'=>  $app_name.'_index','body' => $bodys, 'operation' => 'Batch', 'type' => '_doc', 'idlist' => $fileIdsArray)), 'elastic');
                        return $bodys;
                    }
                }
            }
